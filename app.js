...
function renderSupplierSelect(){ const selected = $('supplierSelect').value; const suppliers = ['Все поставщики', ...new Set(state.stock.map(r=>r.supplier).filter(Boolean)), ...Object.keys(state.suppliers)].filter(Boolean); const unique=[...new Set(suppliers)].sort((a,b)=>a.localeCompare(b,'ru')); $('supplierSelect').innerHTML = unique.map(s=>`<option ${s===selected?'selected':''}>${s}</option>`).join(''); }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  $('menuBtn').addEventListener('click', openDrawer); $('closeMenuBtn').addEventListener('click', closeDrawer); $('drawerBackdrop').addEventListener('click', closeDrawer); $('resetBtn').addEventListener('click', resetAll);
